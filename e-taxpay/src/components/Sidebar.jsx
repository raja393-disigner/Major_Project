...
export default function Sidebar({ type = 'user', isOpen = true }) {
    ...
    return (
        <aside className={`sidebar animate-slide-in ${isOpen ? 'open' : ''}`}>
            <div className="sidebar-menu">
                {items.map((item, index) => {
                    ...
                    if (type === 'admin' && item.section !== lastSection) {
                        ...
                        sectionHeader = (
                            <div className="sidebar-section-title animate-fade-in" style={{ animationDelay: `${index * 0.05}s` }} key={`section-${item.section}`}>
                    ...
                    return (
                        <div key={item.key} className="anim-fade" style={{ animationDelay: `${index * 0.1}s` }}>
                            {sectionHeader}
                            <button
                                className={`sidebar-item hover-lift ${isActive ? 'active' : ''}`}
                                onClick={() => navigate(item.path)}
                                style={{ position: 'relative' }}
                            >
                                <Icon className="icon" size={18} transition="all 0.3s" />
                                <span style={{ transition: 'margin-left 0.3s' }}>
